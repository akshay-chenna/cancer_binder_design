...
do
	for j in {1..2}
	do
		CUDA_VISIBLE_DEVICES=$i python /paperspace/apps/BindCraft/bindcraft.py --settings "./trac_${j}.json" --filters './default_filters.json' --advanced './betasheet_4stage_multimer.json' &
	done
done
wait
