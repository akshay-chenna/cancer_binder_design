...
do
	for j in {1..2}
	do
		CUDA_VISIBLE_DEVICES=$i python /paperspace/apps/BindCraft/bindcraft.py --settings "./gpc3_1.json" --filters './default_filters.json' --advanced './hotspots10_4stage_multimer.json' &
	done
done
wait
