...
install_dir=$(pwd)
CONDA_BASE=$(micromamba info | grep 'root prefix' | cut -d ":" -f 2 | xargs) || { echo -e "Error: mamba is not installed or cannot be initialised."; exit 1; }
...
micromamba env list | grep -w 'BindCraft' >/dev/null 2>&1 || { echo -e "Error: Conda environment 'BindCraft' does not exist after creation."; exit 1; }

# Load newly created BindCraft environment
echo -e "Loading BindCraft environment\n"
micromamba activate BindCraft || { echo -e "Error: Failed to activate the BindCraft environment."; exit 1; }
micromamba env list | grep -w 'BindCraft' >/dev/null 2>&1 || { echo -e "Error: The BindCraft environment is not active."; exit 1; }
...
for pkg in "${required_packages[@]}"; do
    micromamba list "$pkg" | grep -w "$pkg" >/dev/null 2>&1 || missing_packages+=("$pkg")
done
