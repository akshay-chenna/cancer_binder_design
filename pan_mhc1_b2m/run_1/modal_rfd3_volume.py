...
def run_parallel(
    ...
    def rfd3_mpnn_rf3(
        ...
        def run_rf3(mpnn_atom_array,mpnn_id):
            ...
            res_starts = get_residue_starts(bb_binder_refolded)
            seq_1letter = ''.join(
                ProteinSequence.convert_letter_3to1(res_name)
                for res_name in bb_binder_refolded.res_name[res_starts]
            )

            rf3_output.summary_confidences['bb_sc_rmsd'] = bb_binder_sc_rmsd.item()
            rf3_output.summary_confidences['bb_target_aligned_rmsd'] = bb_binder_target_aligned_rmsd.item()
