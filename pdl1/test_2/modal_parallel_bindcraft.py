...
modal run modal_parallel_bindcraft.py \
    --settings \
    --filters \
    --advanced \
    --pdb-in \
...
@app.function(gpu=GPU, timeout=TIMEOUT) 
def run_bindcraft(
    settings_str: str,
    filters_str: str,
    advanced_str: str,
    ...
    with TemporaryDirectory() as in_dir:
        ...
        settings_json["design_path"] = str(in_dir)
        ...
        pdb_path = pdb_path.write_text(pdb_str)
        print(pdb_path)
        
        work_path = Path(in_dir)
        ...
        return [
            (f"Job_{job_idx}/{out_file.relative_to(work_path)}", out_file.read_bytes())
...
def main(
    ...
    print(f"All BindCraft jobs completed. Outputs saved to {out_dir}.")